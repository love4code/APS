...
exports.list = async (req, res) => {
  try {
    ...
    for (const period of payPeriods) {
      ...
      period.totalGrossPay = records.reduce((sum, r) => sum + (r.totalGrossPay || 0), 0)
      period.totalHours = records.reduce((sum, r) => sum + (r.totalRegularHours || 0) + (r.totalOvertimeHours || 0), 0)
    }
...
exports.detail = async (req, res) => {
  try {
    ...
    const timeEntries = await TimeEntry.find({
      ...
      .populate('employee', 'firstName lastName hourlyRate annualSalary payType defaultOvertimeMultiplier')
      .sort({ employee: 1, date: 1 })
      .lean()

    // Get payroll records if processed
    const payrollRecords = await PayrollRecord.find({ payPeriod: payPeriod._id })
    ...
    const summary = {
      totalEmployees: new Set(timeEntries.map(e => e.employee._id.toString())).size,
      totalRegularHours: timeEntries.reduce((sum, e) => sum + (e.type === 'regular' ? e.hoursWorked : 0), 0),
      totalOvertimeHours: timeEntries.reduce((sum, e) => sum + (e.overtimeHours || 0), 0),
      totalPTOHours: timeEntries.reduce((sum, e) => sum + ((e.type === 'pto' || e.type === 'sick') ? e.hoursWorked : 0), 0),
      totalGrossPay: payrollRecords.reduce((sum, r) => sum + (r.totalGrossPay || 0), 0)
    }
  ...
  try {
    ...
    for (const empId in employeeTimeMap) {
      ...
      if (employee.payType === 'hourly') {
        ...
        const overtimePay = totalOvertimeHours * (employee.hourlyRate || 0) * overtimeMultiplier
        ...
        const daysInPeriod = Math.ceil((payPeriod.endDate - payPeriod.startDate) / (1000 * 60 * 60 * 24)) + 1
