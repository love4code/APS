...
exports.newForm = async (req, res) => {
  try {
    ...
    const products = await Product.find({ isActive: true }).sort({ type: 1, name: 1 });
...
exports.updatePayment = async (req, res) => {
  try {
    ...
    await ActivityLog.create({
      ...
      details: job.isPaid ? `Marked as paid on ${job.datePaid}` : 'Marked as unpaid'
    });
